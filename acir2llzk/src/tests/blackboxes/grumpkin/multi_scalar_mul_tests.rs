...
fn translate_multi_scalar_mul_module<'d>(
    driver: &'d Driver,
    circuit: acir::circuit::Circuit<acir::FieldElement>,
) -> Module<'d> {
    let module = translate_single_circuit_module(driver, circuit).expect("translation should pass");
...
fn multi_scalar_mul_translates_and_verifies() {
    ...
    let module = translate_multi_scalar_mul_module(&driver, circuit);
    println!(
