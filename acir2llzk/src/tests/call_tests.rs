...
fn translate_and_assert(
    ...
    for (i, callee) in calls.iter().enumerate() {
        info.calls[i].assert_shape(driver.context(), i, callee, label);
    }
...
fn call_output_used_in_subsequent_solve() {
    ...
    translate_and_assert(&driver, &program, "Circuit0", 3, 3, 4, &["Circuit1"]);
}
...
fn transitive_calls() {
    ...
    c0_info.calls[0].assert_shape(driver.context(), 0, "Circuit1", "Circuit0");
    c0_info.calls[1].assert_shape(driver.context(), 1, "Circuit2", "Circuit0");
    ...
    c1_info.calls[0].assert_shape(driver.context(), 0, "Circuit2", "Circuit1");
...
fn call_only_circuit_verifies() {
    ...
    translate_and_assert(&driver, &program, "Circuit0", 1, 1, 1, &["Circuit1"]);
}
...
fn call_with_nontrivial_predicate() {
    ...
    info.calls[0].assert_shape(driver.context(), 0, "Circuit1", "Circuit0");
...
fn call_with_output_arity_mismatch_returns_error() {
    ...
    let program = make_program(vec![circuit0, circuit1]);
    let result = driver.translate(&program);
    assert!(
        matches!(
            result,
...
fn call_with_multiple_outputs() {
    ...
    let program = make_program(vec![circuit0, circuit1]);
    translate_and_assert(&driver, &program, "Circuit0", 3, 3, 5, &["Circuit1"]);
}
...
fn call_with_public_params() {
    ...
    let program = make_program(vec![circuit0, circuit1]);
    translate_and_assert(&driver, &program, "Circuit0", 2, 2, 3, &["Circuit1"]);
}
