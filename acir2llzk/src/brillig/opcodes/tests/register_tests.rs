...
fn brillig_const_field_emits_felt_const() {
    ...
    let module = translate_body(&driver, vec![const_field(0, 5), brillig_stop()])
...
fn brillig_const_int_accepts_all_bit_sizes() {
    ...
    for (bs, v) in sizes {
        let driver = Driver::new(&TestConfig);
        let module = translate_body(&driver, vec![const_int(0, bs, v), brillig_stop()])
...
fn brillig_mov_emits_load_store_pair() {
    let driver = Driver::new(&TestConfig);
    let module = translate_body(&driver, vec![const_field(0, 7), mov(1, 0), brillig_stop()])
        .expect("translation should succeed");
