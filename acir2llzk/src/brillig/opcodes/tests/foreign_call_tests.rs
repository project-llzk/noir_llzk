...
fn foreign_call_empty_destinations_emits_nothing() {
    ...
    let module = translate_body(&driver, vec![foreign_call_empty("print"), brillig_stop()])
...
fn foreign_call_single_field_destination_emits_one_nondet() {
    ...
    let baseline = count_stores(
        &translate_body(&driver, vec![brillig_stop()])
...
fn foreign_call_heap_array_destination_emits_per_slot_nondet() {
    ...
    let module = translate_body(&driver, body).expect("translation should succeed");
...
fn foreign_call_heap_array_runtime_pointer_emits_per_slot_nondet() {
    ...
    let baseline =
        translate_body(&driver, vec![brillig_stop()]).expect("baseline translation should succeed");
