...
impl<'c, 'p> CircuitTranslator<'c, 'p> {
    ...
    pub(crate) fn translate<'a>(
        ...
        let struct_def = r#struct::def(
            &OpBuilder::at_block_end(self.context, dest),
            location,
            &struct_name,
            |builder| -> Result<(), Error> {
                let compute = r#struct::helpers::compute_fn(
                    builder,
                    location,
                    struct_type,
                    &inputs,
                ...
                let mut compute_writer =
                    BlockWriter::for_compute(self.context, compute, &input_witnesses)?;

                let constrain = r#struct::helpers::constrain_fn(
                    builder,
                    location,
                    struct_type,
                    &inputs,
                ...
                let mut constrain_writer =
                    BlockWriter::for_constrain(self.context, constrain, &input_witnesses)?;

                self.emit_witness_members(builder, &input_witnesses, &opcode_witnesses)?;
                for op in &ops {
                    // Phase 1: struct members
                    op.emit_member(self.context, builder)?;
                    // Phase 2: @compute
