...
#[cfg(test)]
mod tests {
    use super::{AES128_ROUNDS, RCON, SBOX};

    #[test]
    fn aes128_cbc_single_block() {
        // AES-128-CBC: key=iv=plaintext = [0..15].
        // CBC XORs plaintext with IV → all zeros, then encrypts zeros with key.
        let key: [u8; 16] = std::array::from_fn(|i| i as u8);
        let iv: [u8; 16] = std::array::from_fn(|i| i as u8);
        let plaintext: [u8; 16] = std::array::from_fn(|i| i as u8);
        let ct = aes128_cbc_encrypt(&plaintext, &key, &iv);
        // AES-128-ECB(zeros, key=[0..15]) is the NIST AES-128 test vector.
        assert_eq!(
            ct,
            [
    ...
    fn aes128_cbc_encrypt(plaintext: &[u8], key: &[u8; 16], iv: &[u8; 16]) -> Vec<u8> {
        ...
        for b in 0..num_blocks {
            ...
            let enc = aes_block_encrypt(&block, &rk);
    ...
    fn key_expansion(key: &[u8; 16]) -> [u32; 44] {
        ...
        for i in 0..AES128_ROUNDS {
            let temp = rk[3 + i * 4];
            let b0 = SBOX[((temp >> 16) & 0xFF) as usize] as u32;
            let b1 = SBOX[((temp >> 8) & 0xFF) as usize] as u32;
            let b2 = SBOX[(temp & 0xFF) as usize] as u32;
            let b3 = SBOX[((temp >> 24) & 0xFF) as usize] as u32;
            let sub_rot = (b0 << 24) | (b1 << 16) | (b2 << 8) | b3;
            rk[4 + i * 4] = rk[i * 4] ^ sub_rot ^ RCON[i];
            for j in 1..4 {
                rk[4 + i * 4 + j] = rk[i * 4 + j] ^ rk[4 + i * 4 + j - 1];
            }
        }
        rk
    }

    fn aes_block_encrypt(block: &[u8; 16], rk: &[u32; 44]) -> [u8; 16] {
        let mut s = [0u32; 4];
        for i in 0..4 {
        ...
        for round in 1..AES128_ROUNDS {
            let mut t = [0u32; 4];
            for col in 0..4 {
                let s0 = SBOX[((s[col] >> 24) & 0xFF) as usize];
                let s1 = SBOX[((s[(col + 1) % 4] >> 16) & 0xFF) as usize];
                let s2 = SBOX[((s[(col + 2) % 4] >> 8) & 0xFF) as usize];
                let s3 = SBOX[(s[(col + 3) % 4] & 0xFF) as usize];
                t[col] = mix_column(s0, s1, s2, s3) ^ rk[round * 4 + col];
            }
            s = t;
        }
        let mut out = [0u8; 16];
        for i in 0..4 {
            let s0 = SBOX[((s[i] >> 24) & 0xFF) as usize];
            let s1 = SBOX[((s[(i + 1) % 4] >> 16) & 0xFF) as usize];
            let s2 = SBOX[((s[(i + 2) % 4] >> 8) & 0xFF) as usize];
            let s3 = SBOX[(s[(i + 3) % 4] & 0xFF) as usize];
            let word = ((s0 as u32) << 24) | ((s1 as u32) << 16) | ((s2 as u32) << 8) | (s3 as u32);
            let word = word ^ rk[AES128_ROUNDS * 4 + i];
            out[4 * i..4 * i + 4].copy_from_slice(&word.to_be_bytes());
        }
        out
    }

    fn mix_column(s0: u8, s1: u8, s2: u8, s3: u8) -> u32 {
        let r0 = gf_mul(s0, 2) ^ gf_mul(s1, 3) ^ s2 ^ s3;
        let r1 = s0 ^ gf_mul(s1, 2) ^ gf_mul(s2, 3) ^ s3;
        let r2 = s0 ^ s1 ^ gf_mul(s2, 2) ^ gf_mul(s3, 3);
        let r3 = gf_mul(s0, 3) ^ s1 ^ s2 ^ gf_mul(s3, 2);
