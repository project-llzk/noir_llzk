...
#[cfg(test)]
mod tests {
    use super::{KECCAK_ROUNDS, RC, ROT_OFFSETS};
    ...
    fn zero_state_matches_known_vector() {
        let state = [0u64; 25];
        let result = keccak_f1600(state);
        assert_eq!(result[0], 0xF1258F7940E1DDE7);
        assert_eq!(result[1], 0x84D5CCF933C0478A);
        assert_eq!(result[2], 0xD598261EA65AA9EE);
        assert_eq!(result[3], 0xBD1547306F80494D);
        assert_eq!(result[4], 0x8B284E056253D057);
    }

    #[allow(clippy::needless_range_loop)]
    fn keccak_f1600(mut state: [u64; 25]) -> [u64; 25] {
        let mut a = [[0u64; 5]; 5];
        for y in 0..5 {
            for x in 0..5 {
                a[x][y] = state[x + 5 * y];
        ...
        for round in 0..KECCAK_ROUNDS {
            ...
            for x in 0..5 {
                for y in 0..5 {
                    b[y][(2 * x + 3 * y) % 5] = a[x][y].rotate_left(ROT_OFFSETS[y][x]);
                }
            }
