...
fn blake3_emits_shared_helper_and_calls_it_from_wrappers() {
    ...
    assert_eq!(count_occurrences(&ir, "function.def @blake3_3"), 1);
    assert_eq!(count_occurrences(&ir, "function.call @blake3_3"), 4);
...
#[test]
fn blake3_emits_distinct_helpers_for_distinct_input_lengths() {
    let context = LlzkContext::new();
    ...
    let circuit = make_circuit_with_opcodes(
        ...
        vec![
            blake3_blackbox(&[0, 1, 2], outputs1),
            blake3_blackbox(&[3, 4, 5, 6, 7], outputs2),
        ],
    );
    ...
    assert!(module.as_operation().verify(), "Module should verify");
    assert_eq!(count_occurrences(&ir, "function.def @blake3_3"), 1);
    assert_eq!(count_occurrences(&ir, "function.def @blake3_5"), 1);
    assert_eq!(count_occurrences(&ir, "function.call @blake3_3"), 2);
    assert_eq!(count_occurrences(&ir, "function.call @blake3_5"), 2);
}
...
fn blake3_does_not_emit_helper_when_unused() {
    ...
    assert!(module.as_operation().verify(), "Module should verify");
    assert_eq!(count_occurrences(&ir, "function.def @blake3_"), 0);
    assert_eq!(count_occurrences(&ir, "function.call @blake3_"), 0);
}
